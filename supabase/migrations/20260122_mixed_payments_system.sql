-- ========================================
-- MIGRACIÓN: Sistema de Pagos Mixtos (Split Payments)
-- Fecha: 2026-01-22
-- Descripción: Permite realizar ventas con múltiples métodos de pago
-- Ejemplo: 18 Bs por QR + 4 Bs en Efectivo
-- ========================================
...
COMMENT ON TABLE sales_order_payments IS 'Registra los métodos de pago utilizados en cada venta. Permite pagos mixtos (split payments) donde un cliente puede pagar con múltiples métodos.';
...
-- ========================================
-- Agregar columna para identificar pagos mixtos
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Mantener payment_method existente para compatibilidad
-- Se llenará con 'Mixto' cuando is_mixed_payment = true

-- ========================================
-- PASO 5: Migrar datos existentes
-- ========================================
-- Crear registros en sales_order_payments para ventas históricas
DO $$
DECLARE
  v_count INTEGER;
BEGIN
  ...
  IF v_count > 0 THEN
    ...
    SELECT 
      so.organization_id,
      so.id,
      so.payment_method,  -- ✅ Usar directamente (ya validado)
      so.total_amount,
      so.order_date
    FROM sales_orders so
    WHERE NOT EXISTS (
...
CREATE OR REPLACE FUNCTION registrar_venta_transaccion(
    ...
    p_items JSONB,
    p_payments JSONB, -- ✅ NUEVO: Array de pagos
    p_order_type TEXT,
...
BEGIN
    -- ========================================
    -- VALIDACIÓN 1: Verificar que payments sea un array
    -- ========================================
    ...
    -- ========================================
    -- VALIDACIÓN 2: Sumar y validar total de pagos
    -- ========================================
    ...
    -- ========================================
    -- PASO 1: Generar número de venta único
    -- ========================================
    ...
    -- ========================================
    -- PASO 2: Insertar orden de venta
    -- ========================================
    ...
    -- ========================================
    -- PASO 4: Procesar items del pedido
    -- ========================================
    ...
    LOOP
        ...
        -- Obtener detalles del producto incluyendo container
        SELECT 
            p.id,
            p.name,
        ...
        -- ========================================
        -- PASO 5: VALIDACIÓN Y DESCUENTO DE INVENTARIO
        -- ========================================
        IF p_status = 'completed' THEN
            ...
            IF v_para_llevar AND v_product.container_supply_id IS NOT NULL THEN
                ...
                -- Registrar movimiento de envase
...
-- 
-- Cambios aplicados:
-- ✅ Tabla sales_order_payments creada
-- ✅ Políticas RLS implementadas
-- ✅ Índices de rendimiento agregados
-- ✅ Columna is_mixed_payment agregada a sales_orders
-- ✅ Datos históricos migrados
-- ✅ Función registrar_venta_transaccion actualizada
--
-- Próximos pasos:
-- 1. Actualizar servicios TypeScript
-- 2. Crear componente PagoMixtoModal
-- 3. Actualizar reportes de caja
