...
import { getClientes, getCatalogoVentas, registrarVenta, crearCliente, getPedidosPendientes } from '../services/ventasService';
...
const initSystem = async () => {
  ...
  const modificarItem = (id: string, accion: 'cortesia' | 'descuento' | 'eliminar') => {
    if (accion === 'eliminar') {
  ...
  const productosVisibles = catalogo.filter(p => (catFiltro === 'all' || (p.category || 'general').toLowerCase().includes(catFiltro)) && p.nombre.toLowerCase().includes(busquedaProd.toLowerCase()));
          ...
          {carrito.length === 0 ? <div className="h-full flex flex-col items-center justify-center text-stone-300"><ShoppingCart size={48} className="mb-2 opacity-50"/><p>Carrito Vacío</p></div> : 
            ...
            <div key={item.id} className="border-b border-stone-100 pb-3">
              <div className="flex justify-between items-start mb-1">
                <div className="flex gap-2"><span className="bg-stone-100 px-2 rounded text-sm font-bold h-fit text-stone-600">{item.cantidad}x</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-end">
            <div className="w-80 bg-white h-full p-4 overflow-y-auto shadow-2xl animate-in slide-in-from-right">
                ...
                {pendientes.length === 0 ? <p className="text-stone-400 text-center text-sm">No hay pedidos en espera</p> : 
                 pendientes.map(p => (
                    <div key={p.id} className="p-4 border border-stone-200 rounded-xl mb-3 hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer transition-all bg-white" onClick={() => toast.info("Funcionalidad de carga completa pendiente de mapeo de items")}>
