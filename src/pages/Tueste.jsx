...
import { 
  Save, Upload, FileJson, Activity, Flame, Settings, Thermometer, Info 
} from 'lucide-react';
import { 
...
import { getInventarioOro, guardarTueste } from '../services/tuesteService';
...
export function Tueste() {
  ...
  // Información del grano seleccionado (para la tarjeta verde)
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // 1. Intentar Parseo JSON
        try {
          data = JSON.parse(text);
        } catch (jsonError) {
          // Limpieza de formato .alog (Python style)
        ...
        // 2. Mapeo de Variables (Basado en tu hc.json)
        // Artisan 3.2+ usa 'timex'
        const times = data.timex || data.time || data.x || [];
        
        // CORRECCIÓN CRÍTICA DE CURVAS:
        // En tu archivo: temp2 tiene la caída (TP) -> Es el GRANO (Bean)
        // temp1 es más estable -> Es el AIRE (Air)
        const beans = data.temp2 || data.bean_temp || []; 
        const airs = data.temp1 || data.air_temp || [];

        if (!Array.isArray(times) || times.length === 0) {
          throw new Error("El archivo no contiene datos de tiempo (timex).");
        }

        // 3. Procesar Puntos (Reducir resolución si es muy grande opcional, aquí tomamos todo)
        ...
        // 4. Procesar Eventos Importantes (Solo Fases)
        const eventosDetectados = [];
        if (data.computed) {
          const c = data.computed;
          // Helper para agregar si existe el tiempo
          const add = (type, time, temp, label) => {
            if (time && time > 0) eventosDetectados.push({ type, time, temp, label });
          };

          add('CHARGE', c.CHARGE_time, 0, 'Carga');
          add('TP', c.TP_time, c.TP_BT, 'TP');
          add('DRY', c.DRY_time, c.DRY_BT, 'Secado');
          add('FCs', c.FCs_time, c.FCs_BT, '1er Crack');
          add('DROP', c.DROP_time, c.DROP_BT, 'Salida');
        }

        // 5. Configurar Eje X (Cada minuto exacto)
        ...
        setEvents(eventosDetectados);
        setXAxisTicks(ticks);
        setFileStats({ duration: maxTime, maxTemp: Math.max(...beans) });

        // Autocompletar datos ambientales si existen
  ...
  const handleSave = async () => {
    if (!config.machineId || !config.greenId || !config.pesoEntrada || !pesoSalida) {
      return alert("Completa todos los datos (Máquina, Café, Pesos).");
    }
      ...
      <div className="flex-1 flex h-full overflow-hidden">
        {activeTab === 'machines' ? <div className="w-full p-8"><MachineForm/></div> : (
          <>
            ...
            {/* PANEL DERECHO: VISUALIZADOR OSCURO */}
              ...
              {dataPoints.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dataPoints} margin={{ top: 40, right: 40, bottom: 40, left: 10 }}>
                    ...
                    {/* CURVAS INVERTIDAS: Grano=Rojo, Aire=Azul */}
                    ...
                    {events.map((ev, i) => (
                      <React.Fragment key={i}>
                        <ReferenceLine x={ev.time} stroke={COLORS.grid} strokeDasharray="3 3" />
                        <ReferenceDot x={ev.time} y={ev.temp} r={4} fill={COLORS.events} stroke="none">
                          <Label value={ev.label} position="top" offset={10} fill={COLORS.text} fontSize={10} />
                        </ReferenceDot>
                      </React.Fragment>
                    ))}
                  </LineChart>
