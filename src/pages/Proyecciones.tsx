...
import { Calculator, Droplets, Flame, Package, DollarSign, FileText, Info } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { toast } from 'sonner';
import { getLotesParaProyeccion, LoteProyeccion } from '../services/proyeccionesService';
...
export function Proyecciones() {
  const { orgId } = useAuth();
  const [lotes, setLotes] = useState<LoteProyeccion[]>([]);
  const [selectedLote, setSelectedLote] = useState<LoteProyeccion | null>(null);
  ...
  useEffect(() => {
    if (orgId) {
      getLotesParaProyeccion(orgId).then(setLotes).catch(console.error);
    }
  }, [orgId]);

  const handleSelectLote = (lote: LoteProyeccion) => {
    setSelectedLote(lote);
    if (lote.rendimiento_lab) {
      setParams(prev => ({ ...prev, rendimiento_trilla: Number(lote.rendimiento_lab?.toFixed(2)) }));
  ...
  const calcularEscenario = (): ResultadoProyeccion | null => {
    ...
    return {
      ...
      totalBolsas, remanente, ingresoBruto, gananciaBruta, margen
    };
  };
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {selectedLote && results ? (
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-stone-200 flex flex-col md:flex-row justify-between gap-4">
              <div>
                <h3 className="text-2xl font-bold text-stone-800 flex items-center gap-2">
                  {selectedLote.codigo} 
                  <span className="text-sm font-normal text-stone-500 bg-stone-100 px-3 py-1 rounded-full">{selectedLote.variedad}</span>
                </h3>
                {selectedLote.notas && (
                  <p className="text-stone-500 mt-2 text-sm flex items-start gap-2 max-w-2xl">
                    <FileText size={16} className="shrink-0 mt-0.5"/> {selectedLote.notas}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                  <label className="block text-xs font-bold text-stone-600 mb-1 flex justify-between">
                    Rendimiento Trilla (%)
                    {selectedLote.rendimiento_lab && <span className="text-blue-600 text-[10px] flex items-center gap-1"><Info size={10}/> Lab</span>}
                  </label>
                ...
                {/* ... (Otros inputs similares: Merma, Peso, Precio) */}
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" className="w-full pl-9 p-2 border rounded-lg font-bold text-stone-700" value={params.merma_tueste} onChange={e => setParams({...params, merma_tueste: parseFloat(e.target.value) || 0})}/>
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" className="w-full pl-9 p-2 border rounded-lg font-bold text-stone-700" value={params.peso_empaque} onChange={e => setParams({...params, peso_empaque: parseFloat(e.target.value) || 0})}/>
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" className="w-full pl-9 p-2 border-2 border-emerald-100 bg-emerald-50/50 rounded-lg font-bold text-emerald-700" value={params.precio_venta} onChange={e => setParams({...params, precio_venta: parseFloat(e.target.value) || 0})}/>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-stone-300">
            ...
            <p className="text-sm mt-2">Selecciona un lote para comenzar.</p>
          </div>
        )}
      </div>
    </div>
