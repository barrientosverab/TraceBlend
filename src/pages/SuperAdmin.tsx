...
import { generarDatosDemo } from '../services/demoService'; // Importar servicio
import { Database } from '../types/supabase'; // Importamos los tipos reales
...
export function SuperAdmin() {
  const { user } = useAuth();
  const [orgs, setOrgs] = useState<OrganizationRow[]>([]); // Tipado estricto
  ...
  const cargarOrganizaciones = async () => {
    ...
    // Ahora el select es type-safe. Si pides una columna que no existe, TS te avisará.
    const { data, error } = await supabase
      .from('organizations')
      .select('*') // Traemos todo para asegurar que tenemos los campos nuevos
  ...
  const extenderSuscripcion = async (orgId: string, meses: number) => {
    ...
    // Ya no necesitamos 'as any'. TypeScript sabe que 'status' y 'plan' 
    // son campos válidos en la tabla 'organizations'.
    ...
    if (error) {
        ...
        toast.success(`Suscripción extendida ${meses} mes(es)`);
        cargarOrganizaciones();
    }
  };
              ...
              {orgs.map(org => {
                ...
                return (
                  <tr key={org.id} className="hover:bg-stone-50 transition-colors">
                    ...
                    <td className="p-4">
                        <span className="uppercase text-xs font-bold text-stone-500 border border-stone-200 px-2 py-1 rounded bg-stone-50">
                            {org.plan || 'TRIAL'}
                        </span>
                    </td>
